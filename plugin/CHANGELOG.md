# Changelog

本文件记录面向使用者的变更。检查清单与判定语义属于公开契约的一部分，故**新增检查、子命令、判定翻转都算 MINOR**。

## 发布规则 / Release policy

| 变更类型 | 版本位 | 例 |
|---|---|---|
| 新增检查、新增子命令/开关 | **MINOR** | 新增 `--boot-check`、新增 S13 |
| 判定或语义变化（同一输入给出不同结论）、默认行为变化、输出结构新增字段 | **MINOR**（并在下方 *Changed* 里逐条写明） | 默认会话目标改为跳过活跃会话；P21 定为 error |
| 纯修复（错判、崩溃、文案出处错误、CI/依赖问题） | PATCH | E4 无安装树时改 skip |
| 新增**远程目录检查**（数据驱动，`checks.json`） | **不动版本** | 6 小时内自动生效，无需发版 |

> 0.x 期间 MINOR 位是"特性/行为"轴。此前的实践曾把特性当成 PATCH 连发（2026-09-15 一天 19 个 PATCH），
> 那会让 `^0.4.x` 的消费者静默收到判定翻转——本文件与 `0.5.0` 就是为纠正这一点。

---

## [0.8.5] — 2026-09-16

### Fixed — P23 判据重构（红队第四轮：6 条反例，2 条高severity）

巡查 #6789 时新增的 P23（插件自带宿主机包副本），判据被红队**打穿三版**后定为 v5。完整档案 `docs/redteam-0.8.5.md`。

| # | 反例 | 处置 |
|---|---|---|
| **A（高）** | `dsh.bundle`/`dsh.client` **不能**用来区分"插件 vs 宿主库" —— 宿主树上 `dsh-base`/`dsh-web-app`/`dsh-client-ui-*` 全带这两个字段，于是"**宿主核心 bundle 的第二份实例**"（更致命）恰好被豁免 | 不再比类型标签，改判 **"同一包名既在 bundle 解析路径上、也能沿祖先链解析到"** |
| **B（高）** | 空目录 / 截断 manifest / 同名普通文件被误报成"宿主包副本" | 只有 manifest 可读且 name 匹配才算副本；读不出的单列为**残留目录** |
| **C（中高）** | 别名（`nm/scope-copy`，真身在 manifest 的 `name`）、深层嵌套、`.pnpm` 实体全漏 | 扫描时**读每个目录自己的 manifest** + 有界递归 6 层；旧语料对"别名"是**假保证**，已重写 |
| **D（中）** | "排除 `_quarantined`"读的是顶层字段，而写入方在 `dsh.profile._quarantined` → 从不生效 | 改读真实位置 |
| **F（低）** | 头注释仍宣称已删除/从未实现的判据（**注释说谎**，正是 C 类漏报来源） | 注释按实际判据重写，并把 v1→v5 弯路写进注释 |
| E | 用户 patch `insert` 加载的插件不被扫 | **未修 → D8** |

### 修的过程中**作者自己引入 4 处新问题**（全部被机制抓出）

`ancestorResolves` 从 bundle 目录开始 → 嵌套副本**自我确认**（F5 对照被误报）；别名目录从未被读到；
`topLines` 作用域错误让**整个 profile 段 P0 崩溃**；语料用 `null` 表示空目录导致驱动抛错。

### 验证矩阵

12 例（**真实布局**：宿主层在 `profiles/node_modules`）全部通过 —— 含 A/A2/F1/C/C2/C3 六种阳性形态，
与 B1/B2/B3/F5/误报对照/D 六种阴性或豁免形态。**fixture 真实性（R9）再次成为关键**：
只放嵌套副本、不放祖先层宿主包，就测不到真实条件。

测试 221 → **224**；语料 21 → **24**；变异 **8 杀死 / 0 存活 / 1 等价**。

## [0.8.4] — 2026-09-16

**巡查产出的新检查**：社区 #6789 报告的形状，用新机制（语料优先 + 变体分离）验证后发现是**真实漏检**。

### Added — P23：插件把宿主机包声明为普通依赖（社区 #6789）

- **报告的现象**：`@deepseek-ai/dsh-experimental-browser-use-runtime` 把 `@deepseek-ai/dsh-scope` 写成
  **dependency**（而非 peer）→ 该包自带**第二份副本** → 两份实例 Symbol/协议不匹配 → **第一个会话能用、之后每个会话都失败**。
- **为什么此前抓不到**：`P5` 只扫 profile **顶层** `node_modules/@deepseek-ai`；未 hoist 时副本**嵌套在插件自己的 node_modules 里**，
  顶层什么也看不到。**用两个变体分别实测确认**：有顶层副本 → P5 命中；**只有嵌套副本 → 没有任何检查提及**
  （这一步很关键：若只看"顶层也有一份"的那个变体，就会误判为"我们已覆盖"——正是本会话反复犯的"用巧合冒充覆盖"）。
- **新检查判据**：已装 bundle 的 manifest 里 `dependencies` 出现 `@deepseek-ai/*` 即 fail（宿主包应由宿主共享根提供），
  并申报 `examined`/`examinedWhat`。对照实测：改用 `peerDependencies` 的插件 **不误报**。
- **配套（W4/W6）**：语料新增两条（病态 + **健康对照**），共 16 条；变异清单新增 `P23-host-dep-detection`
  （把检出判据改成恒假，语料必须变红）。

测试 214 → **218**；变异 **9 个 → 8 杀死 / 0 存活 / 1 等价**。

## [0.8.3] — 2026-09-16

**把"自造的对抗者"变成默认动作**（用户要求：红队入流程 + 不依赖外部报错也能自我进化）。

### Added —— 变异测试：验证"守卫是否真的在守"

`scripts/mutation-test.mjs`：**故意破坏每个守卫，要求测试必须变红**；破坏后测试仍绿 = 那个守卫是装饰品。

- 首轮结果：**8 个变异中 4 个存活** —— 也就是说**一半守卫此前无人看守**：
  `arity-guard`（没测）、`catalog-skip-as-pass`（没测）、`closure-symbol`（我的变异插错了位置）、
  `coverage-reset-noop`（**守卫本身冗余**：各段都自己设了值，去掉重置没有任何测试变红）。
- 处置：补了两条缺失的测试（arity 守卫 / 目录检查 skip 语义，且**用例自控环境**：PATH 收窄到只有 node，
  否则 E7 在有 dsh 的机器上合法地 pass，测试就变成对环境的断言）；把"覆盖量上下文"改为**绑定求值阶段**
  （`coverageNow()` 只接受属于当前阶段的上下文），守卫才有牙齿；无牙齿的那条如实标为**等价变异**并记录。
- 终态：**杀死 7 / 存活 0 / 等价 1（已记录）**。

### Added —— 发布闸门：红队不跑就不许发版

- `docs/redteam-brief.md`：红队行动简报（如何发起、攻击面、产出要求）。
- `scripts/release-check.sh`：五道闸门 —— 环境泛化审计 · **变异测试无存活** · 全套测试 ·
  **本版本必须留下 `docs/redteam-<版本>.md`** · 待决策清单可见。缺一项即"不许发布"。
- 接入 CI（ubuntu × node24）。**闸门第一次运行就拦住了 0.8.3 自己**（缺红队档案）。

### Added —— 缺陷台账：让"自我进化"可度量

`docs/defect-ledger.md`：20 条缺陷按**来源**分类（EXT 外部报告 / RT 红队 / MUT 变异 / AUDIT 自审），
并给出**预发布发现率**。截至今日：EXT 6 · RT 6 · MUT 1 类 · AUDIT 7 → **预发布发现率 ≈ 70%**。
台账里如实注明：**红队那 6 条是因为用户要求才跑的**，所以闸门（强制留下档案）才是把比率变成能力的那一步。

## [0.8.2] — 2026-09-16

**红队审计（fresh-context 证伪者）在"已自审三轮"的版本里找出 6 个真漏洞**，其中 2 个推翻了我刚声称"已验证"的断言。
完整档案：`docs/redteam-2026-09-16.md`；未修项见 `docs/check-authoring-rules.md` §5。

### Fixed

- **目录检查把"跳过"报成 pass，并伪造覆盖量**（最严重）：`runCatalogCheck` 自报 `skipped` 被记成 `ok:true`；
  `report()` 的 `examined` 回退到段级占位值 1，零对象永不降级。实测 `✓ 未发现 DSH 环境…跳过 dsh 的 PATH 检查`；
  profile 目录**整个不存在**时 P6 仍 pass。
  - 自报 `skipped` → `reportSkip`；探测原语的"目标不存在"分支补 `skipped: true`（根因是**分支里没设标记、文案却写着"跳过"**）；
    保留"文案含跳过 ⇒ skip"作兜底。空环境下被伪造的 `verified` 由 11 降到 8。
- **闭集检测有盲点**：`⚠` 也是判定符号却裸写在收口点外，而闭集正则只有 `/[✓⊖✗]/`。纳入 `⚠` 后**立刻又暴露 3 处**，已全部收口；
  判定符号正则集中为 `VERDICT_GLYPH_RE`；**收口处剥掉数据里的判定符号**（远程目录数据不得伪造判定上屏）。
- **`reportSkip` 缺 `phase`** → 补上（"单位属于求值阶段"这条断言此前在 skip 上无法核对）。

### Changed — 契约级决策，**未擅改**

- **退出码随输出格式分叉**（`--json` 1 / `--envelope` 2 / 文本 1）：契约 `doctor-contract.md:32` 规定
  `0 全通过 / 1 任何 WARN / 2 任何 FAIL` → **envelope 是对的一边，主路径违反契约**。
  我一度把 envelope 改成 1（**统一到违反契约的一边**），已回滚；分歧现由测试钉住并列为待决策 D1。

### Infra

- `scripts/audit.sh` 的畸形-manifest 断言由 `n > 1` 收紧为 `n >= 15`，并把当前不达标**如实计为"已知缺口"**（自证太弱是 R8 的实例）。
- 新增 `docs/redteam-2026-09-16.md`（逐条反例 + 处置 + 红队未能验证项）。

### 第二轮红队（攻击这批新机制本身）→ 本轮修复

完整档案 `docs/redteam-0.8.3.md`。**8 条真反例**，其中 1 条推翻台账里"R1 已修 0.8.2"的结论：

- **F1（严重）**：0.8.2 只修了**目录段**的"跳过报成 pass"；**profile 段的阶段占位值仍在伪造覆盖量** ——
  3 个**一个都没装**的幽灵 bundle 也能得到 `✓ 16 项通过 / ok=true / verified=16`，且 15 条记录带
  `examined=3, examinedWhat="bundle 条目"`（P21 文案写"扫描 0 个 host 入口"却记 examined=3）。
  **已修**：删除占位回退（`examined` 只能显式申报）；未申报者标 `unreported` 并**单独计数、进 JSON**；
  补齐 P11/P15/P17/P19/P20/P21 的数量与单位；删除"数量巧合就继承单位"的回退（它会把 P15 误贴成"3 个 bundle 条目"）。
- **F3/F4/F8（高）**：发布闸门可被空文件或任意版本号旁路；**闸门与变异清单此前根本未入库**；闸门诊断在 C locale 下崩溃且从不核对数量。已修并入库。
- **F5（高）**：测试**依赖网络**导致不可复现，进而让变异测试产生**假杀死**（环境红 = 记成 kill）。
  已加离线开关 `DSH_DOCTOR_OFFLINE=1`；变异判据收紧为"必须出现断言失败"；每条变异先跑基线；超时单列。
- **F6（中）**：等价变异是自证后门（无预算、不进 `--json`）。已加预算（1）并在 JSON 暴露。
- **F7（中）**：台账计数与自己的 20 行不自洽 → 已按行重算并写明归属规则。
- **F2（严重，未修）**：闭集只是**文本律**（`String.fromCharCode` 可绕过；`client.js` 未被扫描）→ **待决策 D6**。
- 附注：`--no-catalog` 只是 `noRemote`，`checks.local.json` 仍会合并 → **待决策 D7**。

测试 144 → **214**。

## [0.8.1] — 2026-09-16

### 0.8.0 的回归审计 —— **修一个引入一个，被查出来了四个**

对 0.8.0 做回归审计（R12/R13），确认它**引入了新问题**：

| 引入的问题 | 现象 | 修法 |
|---|---|---|
| **覆盖量上下文跨段泄漏** | 会话段的 S6/S14 号称"检查了 **16 个 bundle 条目**"，而它们实际展开的是 **11352 个事件** —— 伪造的数字比没有数字更危险，因为它看起来可信 | 段边界 `resetCoverage()` + **单位属于求值阶段**（新增 `phase` 字段）；回归测试断言"单位不得串台" |
| **目录段继承会话单位** | 目录检查 `P6-patch-name-space`（section=profile）声称"检查了会话日志" | catalog 阶段独立单位（`目录检查项`） |
| **R6 只校验类型、不校验个数** | JS 静默忽略多余参数 → 我给 S11 补覆盖量时多传一个 `undefined`，**没人报错**（类型守卫也抓不到） | `report()` 增加**参数个数守卫**（>7 即抛） |
| **两处测试自伤** | 抽取 `report` 的测试缺 `currentPhase` 桩；清单需重新生成 | 已修 |

### Added — 论坛病例语料（`plugin/test/forum-cases.test.mjs`）

把社区**实际报告的问题**做成回归用例（R2 样本律），每例都配**健康对照**，同时防漏报与误报：

| 病例 | 出处 | 结果 |
|---|---|---|
| 缺 `dsh.bundle` → `--boot-check` 必须失败 | #6788 | ✓ 通过（含对照） |
| manifest 带 BOM → P15 报出且其余检查仍运行 | #6758 | ✓ 通过（含对照） |
| 迁移后缺 `data.message` → S14 报出 | #6686 | ✓ 通过（含对照） |
| 裸引用 `harness`/`styles` → P21 失败 | #6693 规则2/4 | ✓ 通过（含注释/字符串对照） |
| client 产物写成裸 ESM → P20 报出 | #6693 规则3 | ✓ 通过（含对照） |
| `apply()` 期抛错 | #6693 规则1 | **已知不能检出**：我们只到 import 层，判定仍是 loadable —— 作为**边界用例**钉在测试里，防止把"P20/P21 通过"误读成"一定能起来" |
| 损坏的 `.zstd` → 判为损坏而非"读取不稳" | #6739 | ✓ 通过。**仅损坏一侧可验证**：报告者的"失败帧偏移每次不同"（读取路径不稳）无法在测试里忠实复现，已在用例注释里写明 |

**写这个语料时我又犯了一次 R9**：病例 G 的 fixture 我写成两层目录，而真实布局是三层
（`sessions/<项目>/<会话>/<日志>`）→ 定位器找不到 → S11 报"未发现会话日志"。**那是 fixture 失真，不是漏报** ——
修成真实布局后，S11 正确报出"1 个损坏会话…3 次尝试均失败 → 倾向于文件本身损坏"。

测试 129 → 141（+12 论坛病例）；审计 **8/8**。

## [0.8.0] — 2026-09-16

### 结构性修复：假绿灯的**类**被消灭，而不是又一个实例

用户指出：**刚复查过又犯错，说明规则本身有缺陷**。复查确认了缺陷的性质——**R3 的强制点选错了**：
它被写在 `report()` 里，而判定并不都在那里产生。机械枚举（而非凭记忆）发现 **21 处代码位置手写 ✓/⊖/✗**，
其中我只加固过 3 处；另外两处从未查过：

| 位置 | 修前行为 |
|---|---|
| `--boot-check` | 零对象仍打印"✓ 所有可探测 entry 均可导入" |
| `--safe-add` | 零对象 `ok:true, stage:'verified'` **并写"已知良好"快照**（把"未验证"固化成"已验证"） |
| `--post-upgrade` | 同样打印"✓ 装载模拟通过"并写快照 |
| **顶层聚合** | 极简 PATH 下 18 skip / 2 pass，仍打印 **"✓ 全部通过"**（从未查过） |
| **JSON 形态** | `ok: true` 只要求"无失败"，"全部跳过"也算通过 |

**改为收口点结构**（R10 边界律 + R11 闭集律）：

- **`makeVerdict(state, opts)`** —— 唯一能构造 `pass` 的地方；`checked === 0` 时**构造期即抛错**（不是渲染期补救）；
- **`printVerdict()` / `printItem()` / `VERDICT_MARKS`** —— **唯一**打印 `✓/⊖/✗` 的三个位置，
  调用点传**状态名**而不是符号（于是符号泄漏在结构上不可能）；
- **`aggregateVerdict()`** —— 聚合判定如实给出 `verified / skipped / failed`：
  全部跳过时输出 `⊖ 没有任何检查实际验证过（N 项全部跳过）——这不等于通过`；
  有跳过时输出 `✓ N 项通过、M 项未检查（skip）——未检查的部分不代表通过`；
- **JSON 契约收紧**：`ok` 只有在**确实验证过（verified > 0）且无失败**时才为 true，并新增
  `verified / skipped / failed` 三个字段。

### Added — 审计与规则的机制化

- **`scripts/audit.sh`**（R12 可重审律 / R13 环境泛化律）：一条命令、五个环境、结果可比——
  空环境 / 极简 PATH（无 dsh、pnpm、zstd）/ 畸形输入（BOM、截断 JSON）/ 清单一致性 / 闭集与全套测试。
  当前 **7/7 通过**。
- **闭集测试**（R11）：断言源码中任何 `✓/⊖/✗` 都必须落在三个收口点内——**今后新加一处手写绿灯会直接红**。
- **规则补 4 条**（`docs/check-authoring-rules.md`）：**R10 边界律**（每条规则必须指明强制点，且是所有同类输出的必经之路）、
  **R11 闭集律**、**R12 可重审律**、**R13 环境泛化律**；§4 记录本次复查，包括**我在这轮审计里又犯的一次 R8**
  （审计脚本第一版断言"空环境 verified 必须为 0"，而空 profile 里 P15 确实验证了 3 个文件——断言强度超过了证据）。

测试 124 → 127。

## [0.7.2] — 2026-09-16

### 用新规则复查旧错误 —— **确认重复犯了同一个错**

按 `docs/check-authoring-rules.md` 的 R1–R9 重新审前面犯过错的案例（尤其审 0.7.0/0.7.1 这批新代码）。
结论：**是的，重复犯了，而且是"治症状"的典型**——我修了 #6788 那个**具体形态**，没修**这一类**。

**① 同一个缺陷出现在三处输出路径**（R3 覆盖律只在 `report()` 里，而这些路径不走它）：

| 路径 | 零对象时的行为（修前） |
|---|---|
| `--boot-check` | 打印"✓ 所有可探测 entry 均可导入"，退出码 0 |
| `--safe-add` | `ok: true, stage: 'verified'` **并写入"已知良好"快照**——把"未验证"固化成"已验证" |
| `--post-upgrade` | 打印"✓ 装载模拟通过"并写快照 |

改为**唯一判定入口 `bootVerdict()`**（三处统一）：零对象一律 `skip`，**不写快照、不宣称通过**。
按 R7 的问法——"如果它完全坏了，会打印什么？"——修前与健康时相同，即**不可证伪**；现在三态可区分。

**② 清单机制不够**（R1/R5）：`docs/check-inventory.md` 曾**缺 8 个动态构造的 id**
（`E1-node`/`E1-pnpm`/`E1-zstd`/`E7-dsh-in-path`/`E8-npmrc-workspace-flag`/`E9-storages-json-valid`/
`E11-settings-writable`/`P6-patch-name-space`），即"查清单"可能查不到它们。现已补齐（**51 项、缺失 0**）。
**范围重叠仍无自动检测**——见下。

**③ 更正我自己的过度声称**（R8 的来历）：0.7.1 的 release note 写"P22 与 P15 重复那件事**由此可被拦住**"，
但该机制只强制一次可见 diff，**并不检测重叠**。已在原处标注更正。

**④ R4 隔离律远未落地**：复查 profile/env 段的读取，**14 处疑似未加保护**，抽查 3 处
（`readFileSync(patchFile)`、`JSON.parse(readFileSync(.../package.json))`、`readFileSync(patchPath)`）
**全部确认在 try 之外**。此前只修了 manifest 那一处。

**⑤ 规则集本身缺两条，已补**（这是复查最有价值的产出）：
- **R8 断言强度律**：断言的强度不得超过证据（复现过 > 读过代码 > 推断；一个样本 ≠ 全体；**机制存在 ≠ 机制足够**）。
  **这是唯一一条没有机制的规则**，已明确标注为待补。
- **R9 版本漂移律**：宿主收紧前置条件后，先假设 **fixture 失真**而不是代码坏了
  （0.1.6-alpha.1 收紧 loader 后，我们多个 fixture 悄悄变成不合法输入，测试还绿着）。

测试 122 → 124。

## [0.7.1] — 2026-09-16

把"事后总结"变成**规则 + 机制**（`docs/check-authoring-rules.md`）。起因是用户指出：一次次的修复是头疼医头，
应当**总结规律、确定规则**。

### 定律（六个错误是它的六个实例）

> **一、结论必须来自来源，不能来自模型；够不到来源就报未知。**
> **二、我们对用户承诺的契约（三态、`unknown` 不折叠、不适用带理由 skip），必须先在工具自身的构建过程中成立。**

### Added — 机制化

- **R6 接口律**：`report()` 对自己的参数做类型校验并**立即抛错**（`examined` 非 number、`src` 非 string 等）。
  来历是我在 `fix` 后多插两个参数、`'builtin'` 落到 `examined` 位置 → 一条本该 pass 的检查被静默降级。
  **落地当天它就抓出了我自己另外 5 处同类错误**——这一类从此可由机制消灭，不必靠小心。
- **R5 唯一归属律 / R1 来源律**：新增 `scripts/gen-check-inventory.mjs` 生成 `docs/check-inventory.md`
  （43 项，含段、范围、来源），并加 CI 断言"清单与代码一致"。
  于是"加检查前先查清单"不再是自觉，而是**一次可见的 diff**。
  ⚠️ **更正（同日复查）**：我最初写的是"P22 与 P15 重复那件事**由此可被拦住**"——**这是过度声称**：
  该机制只强制一次可见 diff，**并不检测范围重叠**；而且清单当时还**缺 8 个动态构造的 id**。
  现已补齐清单（51 项、缺失 0），并把"机制存在 ≠ 机制足够"写成了规则 R8。
  清单中**尚有 8 项未标注权威来源（R1 欠账）**，如实列出。
- **R3 收尾**：环境段补齐覆盖量（每个环境探针各检查一个对象），**全环境覆盖量欠账 18 → 0**；
  CI 断言"空环境下未报告覆盖量的 pass 必须为 0"（此前只是打印数字）。

### 规则集（`docs/check-authoring-rules.md`）

R1 来源律 · R2 样本律 · R3 覆盖律 · R4 隔离律 · R5 唯一归属律 · R6 接口律 · R7 可证伪律。
**每条都必须配一个可执行机制；没有机制的规则一律视为不存在**（这是 §0 第 4 条，也是这一轮最贵的一课）。
文件末尾如实列出各规则的落地状态：R3/R6 已落地，R4/R7 部分落地，**R1/R2/R5 的机制刚起步**。

## [0.7.0] — 2026-09-16

一次**针对工具自身可信度**的加固，起因是连续四轮社区报告都照出了同一个病：**我们的结论比证据更自信**。

### Added — 覆盖不变量（"没看" ≠ "通过"）

实测：空环境（零 bundle、无会话库）下曾有 **18 项 pass，其中 14 项连一个数量都没给** —— "我什么都没看"与
"我全看过、很干净"打印出来完全一样。`--boot-check` 对"bundle 缺 `dsh.bundle`"（零 entry 可探）报
"✓ 全部可导入"（#6788 正是这一类）只是同一 bug 的一个实例。

现在：
- **`examined === 0` 的 `pass` 一律自动降为 `skip`**（"无可检查对象，未做任何比较"）——硬不变量，CI 有测试盯着；
- `pass` 需报告**检查了多少对象**（含 `examinedWhat`）；未报告者带 `coverage: 'unreported'` 欠账标记，
  可被测试度量。**profile 段的欠账已从 18 项清零**（上下文默认值 + 逐项复核单位不同的检查，如
  `P12` 的对象是"已装候选包"、`P18` 是"manifest"，都不是 bundle 列表长度）。
  **env 段仍有 10 项未报告覆盖量**（`E1-*` / `E3-node` / `E4` / `E5` / `E6` / `E10` / `E12` / `E13`）——
  这些探针各查一个对象（某个二进制/版本/端口），补起来是机械的，但**尚未补**，故在此如实列出，
  而不是让 release note 显得比实际干净。

### Fixed — 故障隔离：一条正确的检查曾被无关崩溃静默

#6758 的 BOM 场景里，**P15 本来就能查、也查对了**；但在修复前，主读取点先抛错 →
`P0: profile 检查异常` → **P15 根本没机会执行**。真实失败不是"缺检查"，而是**"正确的检查被上游故障静默了"**。
今天同类问题出现两次（另一处是变量作用域错误掐掉整个 profile 段）。现在外部读取一律不抛（带 BOM 感知的
读取已覆盖 manifest），并有两条回归测试钉住：**BOM 时 P15 仍报出、manifest 无法解析时其余检查仍给结论**。

### Removed — 撤销 `P22`（与 `P15` 重复）

`P15`（关键文件 BOM，源自 #5176）**早就扫描 profile 的 `package.json`** 并报出同一结论；我加 `P22` 之前
**没有查现有清单**。#6758 特有的信息（PowerShell 5.1 的 `-Encoding UTF8` 会写 BOM、`JSON.parse` 硬失败、
报错被 GBK 渲染成乱码）已并入 `P15`。**同一事实只应有一个归属。**

### 为什么这些错误能发生（写下来，供以后对照）

六处错误（S14 判错视图、SR5 成片误报、`--boot-check` 假绿灯、P22 重复、P18 矛盾结论、P15 被静默）**根因相同**：
**用一个"看起来合理"的模型，代替了权威来源**——来源分别是宿主代码路径、整库样本、loader 前置条件、
自家检查清单、宿主的解析代码、检查的执行顺序。

我们的原则（`unknown` 不得折叠、不适用就带理由 skip、"只在一个平台验证过 ≈ 没验证"）此前只写在文档与
评审意见里，**没有变成引擎里的不变量**；而唯一一次我把原则写成机制（peer-range 一致性测试从**已发布源码**
抽函数来跑语料）恰恰没出问题。**原则不落成机制，就只剩运气。**

## [0.6.1] — 2026-09-15

### Fixed — `--boot-check` 在"bundle 缺 `dsh.bundle`"这一类上给假绿灯（社区 #6788）

#6788 报告：0.1.6-alpha.1 起 loader **严格要求** `dsh.profile.bundles` 里每个包声明 `dsh.bundle.patch`，
而 `@deepseek-ai/dsh-computer-use` 等包发布时**漏了这个字段** → 照文档 `dsh plugin add` 之后 profile **立刻起不来**。

这一类的特点是：**没有任何 entry 可探**（包没有 patch，自然没有 insert 条目），于是只做 import 的
`--boot-check` 会输出"✓ 所有可探测 entry 均可导入"——而用户恰恰是按我们的建议先跑它的。**假绿灯出现在
最该给出结论的时刻**，比不报更糟。

现在 `--boot-check`（及其同步版，供 `--safe-add` 使用）在探测 entry 之前先判 **bundle 级前置条件**：
列出的包若解析得到、却没有 `dsh.bundle.patch` → 直接判失败，给出 `missing-bundle-manifest`、
修复方向与隔离命令。宿主核心包（`dsh-base`/`dsh-web-app`，由 CLI 提供）照旧跳过。

（`P1` 早已覆盖同一类：`bundle 条目 X 存在但未声明 dsh.bundle`——本次修的是**我们最推荐的那条自救路径**
没有覆盖它。）

## [0.6.0] — 2026-09-15

### Added

- **P22 profile manifest 带 UTF-8 BOM**（社区 #6758，**error**）：DSH 直接 `JSON.parse` 该文件，带 BOM 时抛 `Unexpected token '…' is not valid JSON` 并**启动硬失败**；而 GBK 控制台会把 BOM 三个字节渲染成乱码，用户看不出是编码问题。判据零误报，补的正是"报错不指向病因"缺的那一句，并给出无 BOM 保存的具体做法（PowerShell 5.1 的 `Set-Content -Encoding UTF8` 默认会写 BOM）。

### Fixed — 我们自己在同一输入上的错判

- 此前直接 `JSON.parse(readFileSync(profile/package.json))`：**带 BOM 时解析失败 → 置 null → P18 报"未找到 profile manifest"**，是**错误结论**；整个 profile 段更会直接报"检查异常"。现在所有 manifest 读取走**带 BOM 感知**的读取：剥离 BOM 后继续工作，`hadBom` 交给 P22 单独报出。**诊断工具不能被它要诊断的那份输入打败**——#6758 让我们发现了自己在同一个输入上的同类问题。

## [0.5.1] — 2026-09-15

### Fixed — 把"读取间歇性失败"错判成"文件损坏"（社区 #6739）

#6739 报告：同一个 25MB 会话每次重读时 `corrupt Zstandard session log: frame at byte N failed validation` 里的 **N 每次不同**，
在 DSH 外手动重试**立即成功** —— 即那不是文件损坏，而是**偶发的读取/校验失败**（存储、内存或驱动层面的抖动）。
宿主不重试（0.1.6-alpha.1 的三处 throw 仍无重试），一次失败就整场历史读不出来。

**我们此前的做法同样糟**：`catch { problems: ['解压/读取失败'] }` → 直接把这种日志记成**损坏**。

现在会话读取带重试，并把三种状态分开：
| 尝试结果 | 判定 | 输出 |
|---|---|---|
| 首次即成功 | 正常 | 照旧 |
| **重试后成功** | **间歇性失败（非损坏）** | 单列一条，说明"文件没坏，是读取路径在抖"，并指向存储健康排查 |
| 三次全失败 | 倾向损坏 | 照旧按损坏处理 |

### Changed — ⚠️ 判定变化

- 此前被判为"损坏会话"的日志，若重试能读出，**不再计入损坏**，改列为"读取间歇性失败"（不翻退出码，避免偶发性抖动导致结果跳动）。

## [0.5.0] — 2026-09-15

累积自 `0.4.6` 的全部变更（中间以 0.4.14–0.4.32 的 PATCH 形式先行发布过，此处归并到 MINOR）。

### Added — 新检查

- **S13 会话头完整性**（#6651）：首行必须是 `{"type":"session"}` 头，且**第一个 zstd 帧恰好只有一行**——判据取自 harness 自身的读取路径（`zlib.zstdDecompressSync` 只解首帧）。此类损坏会让 `dsh web` **整体启动失败**。
- **S14 投影安全性预检**（#6686）：迁移**成功但打开抛错**的会话——缺 `data.message` 的 `assistant/message`/`tool/result`/`system/message`，以及 replace 形态 `surfaceOp` 引用解析不到。对需要迁移的日志**走真实迁移链取"迁移后视图"**再判定，并在输出里注明判定视图。
- **E12 运行时 zstd 稳定性**（#6651）：会话持久化直接依赖 `node:zlib` 的 zstd；用子进程探测是否仍标为实验性。
- **E13 CLI 静默失效签名**（#6341 / #6692）：`dsh` 在 PATH 中但 `--version` **零输出且退出码 0** —— `import.meta.main` 门控（该 API 仅 Node ≥22.18.0 / ≥24.2.0 存在）。
- **P18 profile manifest 的 version**（#6667）：有 `name` 无 `version` 时，解析游离本地模块会抛 `must declare non-empty name and version`。
- **P19 host peer 范围 vs 实际提供版本**（#6678 / #6683，社区 @ciceroyang 提案）：只取 `@deepseek-ai/*`、跟随软链接、作用域名两段、三态（未知既不算兼容也不算不兼容）；另按规范规则 1 计数"有 bundle 却未声明 host 范围"的包。
- **P20 client 产物加载格式**（#6693 规则 3）：必须是 `__ModuleLoader__.load()` 的 CJS 工厂；写成裸 ESM 会在浏览器抛 `Unexpected token 'export'` 而**服务端日志零痕迹**（`node --check` 抓不到，因为那是合法 ESM 语法）。
- **P21 沙箱专属符号**（#6693 规则 2/4，**error**）：host 侧裸 `harness`、client 侧 `styles` / `ctx.get('host')` —— 从 `node_modules` 加载的普通插件里一律不存在；host 侧在 `apply()` 内同步抛出会**中断整棵装载链**。
- E10 增加"端口绑着但 **HTTP 无应答**"判定（#6693）：端口占用 ≠ 服务健康（崩溃重启循环下两者同时成立）。

### Added — 子命令（不依赖 dsh 能启动）

- **`--boot-check`**：离线模拟插件树装载，逐条 entry 真去 import，归类失败（缺导出/未安装/原生 ABI/模块格式/卡死）并给出修复方向与隔离命令。
- **`--quarantine <包>` / `--unquarantine <包>`**：把 bundle 移出启动列表（**先写 `package.json.bak.<ts>`**，只改列表、不动包文件，可撤销）。
- **`--safe-add <包>`**：安装 → 立即验证 → 通过则写"已知良好"快照；失败自动隔离；隔离不足则**整体回滚**到安装前 manifest。
- **`--pre-upgrade` / `--post-upgrade`**：升级前记基线（含核心版本），升级后自动对比 + 复检（`--auto-quarantine` 可一步隔离）。

### Changed — ⚠️ 行为变更

- **运行时检查默认跳过"正在写入的活跃会话"**：原先扫"最新会话"通常就是当前会话，会把操作者自己的操作报成安全发现。现取 mtime 早于活跃窗口的最新会话；窗口用 `DSH_DOCTOR_LIVE_WINDOW_MS` 调整（默认 120000，设 `0` 关闭保护）。`--session` 显式指定始终尊重。
- **`--json` 每条检查新增 `status` 字段**（`pass`/`warn`/`fail`/`skip`，与 `--envelope` 同一词汇表）。纯新增，但消费者此前自行推导的状态应与之一致。
- **P21 按 error 定级** → 某些环境会**新出现 exit 2**；P20 按 warn（判据较粗，不阻断）。
- **P19 的版本判定按外部参考实现对齐**（#6683）：`>=0.1.0-rc.5 <0.2.0` 面对 `0.1.5-rc.2` 由"未知"变为**兼容**；纯 release 区间面对数值满足的预发布版本记**未知**。此前按这一条报出的结论会消失。
- **E3 的支持范围可溯源**：默认标注"内置常量（出处：仓库根 package.json engines.node，核对日期）"；设置 `DSH_DOCTOR_HARNESS_ROOT` 时**真读**该检出。
- `--quarantine` / `--unquarantine` 默认输出人读形态（`--json` 时保留机器形态）。

### Fixed

- **P3：ESM-only 包被误判为不可解析**（#1719 社区指出的 `require.resolve` 陷阱，实测命中我们）——改为存在性判据（跟随软链接、作用域名两段、排除 `cordis:`/相对路径）。
- **E4：无安装树可查时 skip 而非 fail**——原先在合成 HOME/干净容器里报"未找到 node-pty"，连带 E2/E5/envelope 三个用例红；套件由 3 红转全绿。
- **E7 / E13：无 DSH 环境时 skip**，探针判断"被告知的环境"而非进程全局 HOME。
- **S13 覆盖 harness 精确判据**：从"首行是不是头"升级为"第一个 zstd 帧恰好一行"；改用与 harness 相同的解压器。
- **S11 不再把首行非会话头的日志报成"健康"**（#6651 类的假阴性）。
- **S14 改判迁移后视图**（#6686）：初版只看磁盘行，会漏掉迁移过程中产生的形态。
- **P20/P21 两轮误报治理**：注释（`//#region styles`）与字符串（`"deepseek-harness"`）曾被当成符号引用——真实 profile 上 9 处命中全是假的；现要求符号处于代码位置且未被本地声明。
- **E3 出处引用两次更正**：先写"root package.json engines"、后改成"不是来自 manifest"，两次都不准——准确说法是"范围确实声明在仓库根的**私有** workspace（`@deepseek-ai/dsh-root`）上，没有任何发布物继承它"。
- `--boot-check` 会执行插件顶层代码（这正是"启动"的语义），故为显式开关；相关说明已写入 README。

### Infra

- **CI 覆盖 ubuntu + windows × Node 22/24**。加入 Windows 后立即抓出 `dsh-security` SP14 的真实平台缺陷（路径分隔符），并暴露出 7 处测试的平台假设。
- README 新增「dsh 起不来怎么办（不依赖其它工具）」与 `--safe-add`／漂移对比两节。

---

## [0.4.6] 及更早

早期版本请见 git 历史与 `docs/`。要点：`dsh-doctor/v1` 信封与 catalog 检查、S11/S12 全会话扫描与迁移拒载预检、P11–P17 系列、Layer C 观察者。
