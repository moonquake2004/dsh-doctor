...
function checkProfile(name) {
  ...
  // P4 file: 依赖悬空
  const dangling = Object.entries(deps).filter(([, spec]) => spec.startsWith('file:')).filter(([, spec]) => !existsSync(join(dir, spec.slice(5))));
...
function checkSession(targetPath) {
  ...
  if (!target || !existsSync(target)) { report('session', 'S0', false, '找不到会话日志', '传 --session <path>'); return; }
  let text;
  try {
