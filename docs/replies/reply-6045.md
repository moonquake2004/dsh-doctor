Thanks — the analysis holds on the installed build, and we can add a second, much smaller store.
...
**The refusal is genuinely side-effect-free.** After the in-memory run, all three artifacts still carry their original bytes and mtimes, and no `.v3` or temp file appeared. `readHeader` returns `migration-required` with a `cwd`, which is why they list and then fail on open.
...
This class is offline-detectable before anything is opened, which is why we're landing S12 in dsh-doctor: it reads the refusal rules out of the installed migration packages and reports affected logs up front — `dsh-doctor --session <path>` for one log, a full run for the store.
