...
\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=1.7cm, box/.style={draw, rounded corners, align=center, font=\small}]
\node[box] (report) {community\\ report};
\node[box, right of=report] (cand) {candidate check\\ (probe + anchor + fixtures)};
\node[box, right of=cand] (cert) {fixture\\ certification\\ (no FP / catches bad)};
\node[box, right of=cert] (reg) {check registry\\ (data, TTL)};
\node[box, right of=reg] (inst) {installed instances\\ ($\le$6h, no release)};
\node[box, below of=report] (up) {harness upgrade};
\draw[->] (report) -- (cand);
\draw[->] (cand) -- (cert);
\draw[->] (cert) -- node[above] {pass} (reg);
\draw[->] (reg) -- (inst);
\draw[->] (inst.north) -- ++(0,0.7) node[above] {feedback} -| (report.north);
\draw[->] (up) -- ++(1.4,0) |- node[midway, below] {anchor introspection} (inst.south);
\draw[->] (up) -| (report.south);
\end{tikzpicture}
