...
certified check. We document 34 checks mapped to 33 community failure
reports, 109 regression tests that have caught real bugs in the tool's
...
\item
  ...
  implementation.
\item
  \textbf{Field experience.} We document 34 checks, 109 regression tests,
...
\textbf{Table 1 --- Check inventory (34 checks, 33 mapped community
reports, as of v0.4.5).}

\begin{table}[H]
\centering
\caption{Check inventory (34 checks, 33 mapped community reports, v0.4.5).}
\label{tab:checks}
\resizebox{\textwidth}{!}{%
...
profile & P2--P5, P7--P11, P13, P14, P15, installed\_bundle & 13 & composition: resolution / collisions / patch syntax / dual-instance / inject semantics / artifacts & [\#1404] [\#1486] [\#1697] [\#1724] [\#1904] [\#1947] [\#1965]\\
...
the chunk-expansion contract (the \texttt{expandRow}/\texttt{seq0+k}
semantics S6 depends on), the \texttt{session/end-seed} literal, and the
\texttt{sourceEventSeqs} field still exist --- \emph{wherever} the current
release keeps them (in \texttt{dsh-session} for the rc-line, in the
\texttt{dsh-session-format-v0-to-v1} migration package after the 0.1.5 split). The same idea appears
...
\emph{Reference:} the 109-test corpus whose isolation property has caught
...
\subsubsection{6.2 The Anchor Tripwire in Action: A Cross-Train Format Migration}\label{the-anchor-tripwire-in-action-a-cross-train-format-migration}

Twenty-seven days after the first field deployment, the harness had moved from
\texttt{0.1.0-rc.6} to \texttt{0.1.5-alpha.1} and, in doing so, changed two
things at once: it migrated from an npx-checkout install to a \textbf{global npm
install}, and it split the session format into a versioned chain
(v0$\rightarrow$v1$\rightarrow$v2$\rightarrow$v3) whose migration logic lives in
separate packages (\texttt{dsh-session-format-v0-to-v1},
\texttt{-v1-to-v2}, \texttt{-v2-to-v3}). Running the unchanged tool against the
new install reproduced, in one afternoon, exactly the failure mode Property 2
exists to prevent:

\begin{itemize}
\tightlist
\item
  \textbf{The locator went blind.} E6 and S8 resolved \texttt{dsh-session} only
  through the \texttt{node\_modules/.bin/dsh} (npx) layout. Under the global
  install the lookup failed, so both checks fell back to a built-in event-type
  table captured at rc.6 --- E6 degraded to a green ``anchors unverified''
  state, and S8 compared sessions against a stale vocabulary.
\item
  \textbf{The consequence was 49 false positives.} Against the stale table, S11
  reported 49 sessions as corrupt because their legacy event types
  (\texttt{assistant/chunk}, \texttt{tool/code-dispatch}) are absent from the
  current \texttt{KNOWN\_SESSION\_EVENT\_TYPES} --- yet the harness reads them
  correctly through the migration chain. A check that had been right on its
  certified train became a false-positive generator on the next one.
\item
  \textbf{The anchor moved rather than disappeared.} The
  \texttt{expandRow}/\texttt{seq0+k} expansion contract S6 depends on is no
  longer in \texttt{dsh-session}; the equivalent disposition table now lives in
  \texttt{dsh-session-format-v0-to-v1}. E6 \emph{failed loudly} on the missing
  anchor instead of reporting green --- and that failure is what directed the
  repair.
\end{itemize}

The fix was structural rather than a one-off patch: \texttt{dsh-session}
resolution now covers npx, global, and profile layouts; when several installs
coexist, the newest supplies the ``current'' contract while the
\textbf{readable} event-type set is the union across installs plus the types
recognized by the installed migration packages (54 current + 15 legacy = 69 on
the observed machine, versus 41 in the stale table); E6's anchors were
re-pointed to wherever the current release keeps them; and a failed location is
now reported as \texttt{skip} (``does not apply'') rather than a silent pass.
Regression fixtures for all three behaviours were added with the fix (109 tests
total, all passing).

Two lessons generalize beyond this ecosystem. First, \textbf{introspection must
itself be version-robust}: reading the installed contract beats hardcoding only
if the contract can still be \emph{found} after the install layout changes.
Second, \textbf{the tripwire's value is in failing loudly}: the brief window in
which E6 reported ``unverified'' as a pass is precisely the window in which the
tool would have shipped confident wrong answers. The same check infrastructure
now also carries a 26-check security layer (dsh-security), evidence that the
lifecycle model is not specific to diagnostics.

\subsubsection{6.3 False-Positive Elimination: Three Rounds on One
...
\subsubsection{6.4 Real-Profile Findings}\label{real-profile-findings}

On the author's production profile, the tool reports a clean bill for
all 34 checks. Notably, the same profile \emph{does} carry plugins that
...
\subsubsection{6.5 Ecosystem Convergence}\label{ecosystem-convergence}
...
symptom families ↔ our 34 checks, with honest coverage marks including
...
(two consecutive release trains: 0.1.0-rc.6 and 0.1.5-alpha.1) and one production profile. The
...
has shipped 34 checks, maintained 109 regression tests, survived a
