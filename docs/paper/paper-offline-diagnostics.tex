...
\section{Offline Diagnostics for Plugin-Based Agent Harnesses: Design,
Check Lifecycle, and Field Experience with DeepSeek
Harness}\label{offline-diagnostics-for-plugin-based-agent-harnesses-design-check-lifecycle-and-field-experience-with-deepseek-harness}

\textbf{Author:} moonquake2004

\textbf{Version:} 1.0 (working draft, 2026-08-15)
...
\textbf{Figure 1 --- dsh-doctor architecture: one check engine, three
output modes, remote catalog with TTL cache.}
...
\bottomrule\noalign{}
\endlastfoot
env (built-in) & E1--E6, E10 & 7 & PATH / files / native binary /
storage / anchors / port & {[}\#1270{]} {[}\#71{]} {[}\#113{]}
{[}\#1219{]} {[}\#1357{]} {[}\#1534{]} {[}\#1719{]} \\
env (catalog) & E7--E9, E11 & 4 & declarative probes
(path/JSON/text/writable) & {[}\#1270{]} {[}23{]} {[}\#1357{]}
{[}34{]} \\
profile & P1--P5, P7--P11 & 10 & composition: resolution / collisions /
patch syntax / dual-instance / inject semantics / artifacts &
{[}\#1404{]} {[}\#1486{]} {[}\#1697{]} {[}\#1724{]} {[}\#1904{]}
{[}\#1947{]} {[}\#1965{]} \\
session & S1, S2, S6--S11 & 8 & log integrity: continuity / replay /
types / containers / drift / heap & {[}\#1363{]} {[}\#466{]}
{[}\#1333{]} {[}\#1497{]} {[}\#1538{]} {[}\#1043{]} {[}\#1469{]}
{[}\#1550{]} \\
\textbf{Total} & & \textbf{29 built-in/catalog + 1 (E10) = 30} & & 18
distinct reports \\
\end{longtable}
}
...
\textbf{Figure 2 --- The check lifecycle: report → candidate → certify →
distribute → verify (property 4), with introspection (property 2)
guarding every upgrade.}
...
\bottomrule\noalign{}
\endlastfoot
dsh-doctor {[}26{]} & user side (env/profile/session) & 30 (25 built-in
+ 5 catalog) & \texttt{-\/-json\ -\/-envelope} & lowercase
pass/warn/fail \\
dsh-plugin-doctor {[}21{]} & author side (pre-publish) + profile
tripwire & manifest/patch/entry/files/build/pack/install/profile-shadow
& v1.6.0 \texttt{-\/-profile\ -\/-json} & lowercase pass/warn/fail \\
dsh-diagnose {[}22{]} & user side (symptom diagnosis,
knowledge-anchored) & 17 \texttt{dsh-symptom.*} families &
\texttt{-\/-doctor-json} & lowercase (adopted) \\
\end{longtable}
}
